...
#if 0    // POSSIBLE REPLACEMENT
typedef struct {
    int count_BLOCKED;
    Process currBlocked[MAX_RUNNING_PROCESSES];
    Process waitingUnblock[MAX_RUNNING_PROCESSES];
} BlockedQueue;
#endif
...
int count_BLOCKED = 0;
int count_waitingUNBLOCK = 0;

void updateBlockedDuration(int timeChange) {
    for (int i = 0; i < count_BLOCKED; i++) {
        blockedQueue[i].blockDuration -= timeChange;
    }
}

void enqueueBlocked(Process p) {
    if (count_BLOCKED < MAX_RUNNING_PROCESSES) {
        p.state = BLOCKED;  // Running -> Blocked
        blockedQueue[count_BLOCKED] = p;
        count_BLOCKED++;
        globalClock += 10;
        updateBlockedDuration(10);
    }
}
...
void enqueue(Process p) {
    ...
    //Global clock incriments
    switch (p.state) {
        case NEW:               // New -> Ready
            break;
        
        case BLOCKED:           // Blocked -> Ready
        case RUNNING:           // Running -> Ready (Time quantum expired)
            globalClock += 10;
            updateBlockedDuration(10);
            break;

        default:
            break;              // Not possible to go from Ready/Terminated -> Ready
    }
    p.state = READY;
}
...
//            globalClock += 10; // Blocked -> Ready
            ...
            updateBus();
            waitingUnblock[count_waitingUNBLOCK] = i;
            count_waitingUNBLOCK++;
...
//            globalClock += 5; // Ready -> Running
            printf("Ready -> Running +5: %i     Process: %s\n", globalClock, currProcess.processName);
            ...
            if (remainingSyscallDuration >= TIME_QUANTUM) {
                ...
                updateBlockedDuration(TIME_QUANTUM);
                ...
                printf("Time Quantum Exceeded Requeued - GLOBAL CLOCK +10 : %i\n", globalClock);
                enqueue(currProcess);
#if 0     
                if (count_READY > 0) {
                    printf("Time Quantum Exceeded Requeued - GLOBAL CLOCK +10 : %i\n", globalClock);
                    ...
                    enqueue(currProcess);
                    globalClock -= 15; // counter 'Running -> Ready -> Running' = 15usec when no other process to run
                    continue;
                }
#endif
                ...
                updateBlockedDuration(remainingSyscallDuration);
                printf("Remaining Syscall Duration added +%i: %i\n",remainingSyscallDuration, globalClock);
...
//                currProcess.state = BLOCKED;
                ...
                if (currProcess.parent != -1) { 
                    ...
                    printf("Global time after first spawn exit: %i\n", globalClock);
                    //exit(0);
...
//                globalClock += 10; // Running -> Ready x 2 (1x for child process, 1x for parent process)
                printf("Spawn Running -> ready for original process: %i\n", globalClock);
...
//                currProcess.state   = BLOCKED;
    ...
    printf("CPU Percentage: %i/%i = %i%%\n", cpuTime, globalClock, (int)round((double)cpuTime / globalClock * 100));
